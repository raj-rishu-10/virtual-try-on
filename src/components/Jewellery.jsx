...
const Jewellery = () => {
    ...
    useEffect(() => {
        ...
        const faceMesh = buildFaceMeshes(canvasRef.current, videoRef.current, processResults, () => { } // Empty draw function for now
        );
    ...
    return (<>
      <Navbar />
      <TryOnLayout titleText="Jewellery" selectionTitle="Select Jewellery:" items={JEWELLERY_ITEMS} selectedItem={selectedItem} onSelectItem={setSelectedItem} otherCategories={OTHER_CATEGORIES} renderItem={(item, isSelected) => (<div className={`p-[2px] rounded-full border-2 ${isSelected ? 'border-custom-blue' : 'border-transparent hover:border-gray-300'}`}>
