...
const Lipsticks = () => {
    ...
    useEffect(() => {
        ...
        const faceMesh = buildFaceMeshes(canvasRef.current, videoRef.current, processResults, (res, can, ctx) => drawLipstick(res, can, ctx, colorRef.current));
    ...
    return (<>
      <Navbar />
      <TryOnLayout titleText="Lipstick" selectionTitle="Select Lipstick:" items={LIPSTICK_SHADES} selectedItem={selectedShade} onSelectItem={setSelectedShade} otherCategories={OTHER_CATEGORIES} renderItem={(shade, isSelected) => (<div className={`p-[2px] rounded-full border-2 ${isSelected ? 'border-custom-blue' : 'border-transparent hover:border-gray-300'}`}>
