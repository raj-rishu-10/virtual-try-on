...
const TryOnLayout = ({ titleText, selectionTitle, items, selectedItem, onSelectItem, renderItem, children, otherCategories, showBeforeAfterBadges = true, }) => {
    const navigate = useNavigate();
    return (<div className="min-h-screen bg-[#F8F9FA] font-sans pb-12">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-24">
        {/* Back Button */}
        <button onClick={() => navigate('/')} className="flex items-center text-gray-800 font-semibold mb-6 hover:text-custom-blue transition-colors text-sm">
        ...
        <div className="bg-white rounded-3xl shadow-sm flex flex-col lg:flex-row overflow-hidden min-h-[600px]">
          ...
          <div className="lg:w-[65%] relative p-4 lg:p-6 flex flex-col">
             <div className="w-full h-full rounded-2xl overflow-hidden relative bg-[#F3F4F6] flex items-center justify-center">
               {showBeforeAfterBadges && (<div className="absolute top-4 left-4 right-4 flex justify-between z-20 pointer-events-none">
          ...
          <div className="lg:w-[35%] p-6 lg:p-8 flex flex-col border-l border-gray-50">
            ...
            <div className="mb-10">
              ...
              <div className="flex flex-wrap gap-4">
                {items.map((item, index) => (<div key={index} onClick={() => onSelectItem(item)} className="cursor-pointer transition-transform hover:scale-105">
            ...
            <div className="mt-auto">
               ...
               <div className="space-y-3">
                 {otherCategories.map((cat, idx) => (<Link key={idx} to={cat.path} className="flex items-center p-4 rounded-xl bg-[#F3F4F6] hover:bg-gray-200 transition-colors">
