...
const SoulSerumAR = () => {
    ...
    useEffect(() => {
        ...
        const faceMesh = buildFaceMeshes(canvasRef.current, videoRef.current, processResults, (res, can, ctx) => drawFoundation(res, can, ctx, colorRef.current + "D0"));
    ...
    return (<>
      <Navbar />
      <TryOnLayout titleText="Foundation" selectionTitle="Select Foundation Shade:" items={FOUNDATION_SHADES} selectedItem={selectedShade} onSelectItem={setSelectedShade} otherCategories={OTHER_CATEGORIES} renderItem={(shade, isSelected) => (<div className={`p-[2px] rounded-full border-2 ${isSelected ? 'border-custom-blue' : 'border-transparent hover:border-gray-300'}`}>
        ...
        {isLoading && !cameraError && (<div className="absolute inset-0 flex items-center justify-center bg-gray-100/50 rounded-2xl z-20">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-custom-blue"></div>
          </div>)}
        {cameraError && (<div className="absolute inset-0 flex flex-col items-center justify-center bg-white p-8 text-center z-30 rounded-2xl">
