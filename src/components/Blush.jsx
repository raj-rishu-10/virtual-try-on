...
const Blush = () => {
    ...
    useEffect(() => {
        ...
        const faceMesh = buildFaceMeshes(canvasRef.current, videoRef.current, processResults, (res, can, ctx) => drawBlush(res, can, ctx, colorRef.current));
    ...
    return (<>
      <Navbar />
      <TryOnLayout titleText="Blush" selectionTitle="Select Blush:" items={BLUSH_SHADES} selectedItem={selectedShade} onSelectItem={setSelectedShade} otherCategories={OTHER_CATEGORIES} renderItem={(shade, isSelected) => (<div className={`p-[2px] rounded-full border-2 ${isSelected ? 'border-custom-blue' : 'border-transparent hover:border-gray-300'}`}>
