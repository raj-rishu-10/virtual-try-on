...
export const drawLipstick = (results, canvas, canvasCtx, color) => {
    if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
        const maskCanvas = createMask(results, canvasCtx.canvas.width, canvasCtx.canvas.height);
        if (maskCanvas) {
            const lipstickCanvas = applyLipstick(maskCanvas, canvasCtx.canvas.width, canvasCtx.canvas.height, color || "#ff0000");
            if (lipstickCanvas) {
